...
void GetStartTime(struct timespec *tbuf){
	int clk;
	clk = clock_gettime(CLOCK_REALTIME, tbuf );
	if(clk)	outputLog("GetStartTime Error !!\n");
}
...
double GetRunTime(const struct timespec *tbuf){
	...
	clk = clock_gettime(CLOCK_REALTIME, &tbuf2 );
	if(clk)	outputLog("GetRunTime Error !!\n");
	//printf("GetStartTime:%ld,%ld\n",(double)tbuf2.tv_sec,(double)tbuf2.tv_nsec);

runtime = (double)(tbuf2.tv_sec*1000 + tbuf2.tv_nsec/1000000.0 - tbuf->tv_sec*1000 - tbuf->tv_nsec/1000000.0);
