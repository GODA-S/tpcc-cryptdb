...
retry:
	...
	for (i_id = 1; i_id <= MAXITEMS; i_id++) {
		...
		i_price = ((int) RandomNumber(100L, 10000L));
...
/*goda*/	
		char tempquery[300];
		sprintf(tempquery,"INSERT INTO item values(%d,%d,'%s',%d,'%s')",i_id,i_im_id,i_name,i_price,i_data);
		if(option_debug)
			printf("query:%s",tempquery);
		int errflg = mysql_query(mysql,tempquery);
		if(errflg) goto sqlerr;
...
retry:
	...
	for (; w_id <= max_ware; w_id++) {
		...
		w_tax = ((int) RandomNumber(10L, 20L));
		w_ytd = 3000000;
...
/*goda*/
		char tempquery[300];
		sprintf(tempquery,"INSERT INTO warehouse values(%d,'%s','%s','%s','%s','%s','%s',%d,%d)",w_id,w_name,w_street_1,w_street_2,w_city,w_state,w_zip,w_tax,w_ytd);
		if(option_debug)
			printf("query:%s",tempquery);
		int errflg = mysql_query(mysql,tempquery);
		if(errflg) goto sqlerr;
...
/*goda*/
		char tempquery[1000];
		sprintf(tempquery,"INSERT INTO stock values(%d,%d,%d,'%s','%s','%s','%s','%s','%s','%s','%s','%s','%s',0,0,0,'%s')",s_i_id,s_w_id,s_quantity,s_dist_01,s_dist_02,s_dist_03,s_dist_04,s_dist_05,s_dist_06,s_dist_07,s_dist_08,s_dist_09,s_dist_10,s_data);
		if(option_debug)
			printf("query:%s",tempquery);
		int errflg = mysql_query(mysql,tempquery);
		if(errflg) goto sqlerr;
...
retry:
	for (d_id = 1; d_id <= DIST_PER_WARE; d_id++) {
		...
		d_tax = ((int) RandomNumber(10L, 20L));
...
/*goda*/
		char tempquery[1000];
		sprintf(tempquery,"INSERT INTO district values(%d,%d,'%s','%s','%s','%s','%s','%s',%d,%d,%d)",d_id,d_w_id,d_name,d_street_1,d_street_2,d_city,d_state,d_zip,d_tax,d_ytd,d_next_o_id);
		if(option_debug)
			printf("query:%s",tempquery);
		int errflg = mysql_query(mysql,tempquery);
		if(errflg) goto sqlerr;
...
retry:
	...
	for (c_id = 1; c_id <= CUST_PER_DIST; c_id++) {
		...
		c_credit_lim = 50000;
		c_discount = ((int) RandomNumber(0L, 50L));
		c_balance = 10;
		//c_balance = -10; goda 
...
/*goda*/
		char tempquery[1000];
		sprintf(tempquery,"INSERT INTO customer values(%d,%d,%d,'%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s',%d,%d,%d,10,1,0,'%s')",c_id,c_d_id,c_w_id,c_first,c_middle,c_last,c_street_1,c_street_2,c_city,c_state,c_zip,c_phone,timestamp,c_credit,c_credit_lim,c_discount,c_balance,c_data);
		if(option_debug)
			printf("query:%s",tempquery);
		int errflg = mysql_query(mysql,tempquery);
		if(errflg) goto sqlerr;
...
/*goda*/
		sprintf(tempquery,"INSERT INTO history values(%d,%d,%d,%d,%d,'%s',%d,'%s')",c_id,c_d_id,c_w_id,c_d_id,c_w_id,timestamp,h_amount,h_data);
		if(option_debug)
			printf("query:%s",tempquery);
		errflg = mysql_query(mysql,tempquery);
		if(errflg) goto sqlerr;
...
/*goda*/
			char tempquery[1000];
			sprintf(tempquery,"INSERT INTO orders values(%d,%d,%d,%d,'%s',NULL,%d,1)",o_id,o_d_id,o_w_id,o_c_id,timestamp,o_ol_cnt);
			if(option_debug)
				printf("query:%s",tempquery);
			int errflg = mysql_query(mysql,tempquery);
			if(errflg) goto sqlerr;
...
/*goda*/
			sprintf(tempquery,"INSERT INTO new_orders values(%d,%d,%d)",o_id,o_d_id,o_w_id);
			if(option_debug)
				printf("query:%s",tempquery);
			errflg = mysql_query(mysql,tempquery);
			if(errflg) goto sqlerr;
...
/*goda*/
			char tempquery[1000];
			sprintf(tempquery,"INSERT INTO orders values(%d,%d,%d,%d,'%s',%d,%d,1)",o_id,o_d_id,o_w_id,o_c_id,timestamp,o_carrier_id,o_ol_cnt);
			if(option_debug)
				printf("query:%s",tempquery);
			int errflg = mysql_query(mysql,tempquery);
			if(errflg) goto sqlerr;
...
/*goda commentout
		...
		for (ol = 1; ol <= o_ol_cnt; ol++) {
			...
			tmp_float = (int) (RandomNumber(10L, 10000L));
...
/*goda*/
				char tempquery[1000];
				sprintf(tempquery,"INSERT INTO order_line values(%d,%d,%d,%d,%d,%d,NULL,%d,%d,'%s')",o_id,o_d_id,o_w_id,ol,ol_i_id,ol_supply_w_id,ol_quantity,ol_amount,ol_dist_info);
				if(option_debug)
					printf("query:%s",tempquery);
				int errflg = mysql_query(mysql,tempquery);
				if(errflg) goto sqlerr;
...
/*goda*/
				char tempquery[1000];
				sprintf(tempquery,"INSERT INTO order_line values(%d,%d,%d,%d,%d,%d,'%s',%d,%d,'%s')",o_id,o_d_id,o_w_id,ol,ol_i_id,ol_supply_w_id,timestamp,ol_quantity,ol_amount,ol_dist_info);
				if(option_debug)
					printf("query:%s",tempquery);
				int errflg = mysql_query(mysql,tempquery);
				if(errflg) goto sqlerr;
