...
)
{
 ...
 outputLog("no0|");
	...
	char            c_last[17];
	char            c_credit[3];
	int           w_tax; //goda float -> int
	int            d_next_o_id;
	int           d_tax; //goda float -> int
	char            datetime[81];
	int            o_id;
	char            i_name[25];
	int           i_price; //goda float -> int
	char            i_data[51];
	int            ol_i_id;
	int            s_quantity;
	char            s_data[51];
	...
	int           amt[MAX_NUM_ITEMS]; //goda float -> int
	int           price[MAX_NUM_ITEMS]; //goda float -> int
	int            stock[MAX_NUM_ITEMS];
	int           total = 0; //goda float -> int
...
#endif
/* goda [ */
	sprintf(tempquery,"SELECT c_discount, c_last, c_credit, w_tax FROM customer, warehouse WHERE w_id = %d AND c_w_id = w_id AND c_d_id = %d AND c_id = %d",w_id,d_id,c_id);
	GetStartTime(&tbuf);
	if(mysql_query(ctx[t_num],tempquery)) goto sqlerr;
	runtime = GetRunTime(&tbuf);
	query_log[0].runCount++;
	query_log[0].runTime += runtime;

	resp = mysql_use_result(ctx[t_num]);
	if((row = mysql_fetch_row(resp)) != NULL){
		c_discount = atoi(row[0]);
...
#endif
/* goda [ */
	sprintf(tempquery,"SELECT d_next_o_id, d_tax FROM district WHERE d_id = %d AND d_w_id = %d FOR UPDATE",d_id,w_id);

	GetStartTime(&tbuf);
	if(mysql_query(ctx[t_num],tempquery)) goto sqlerr;
	runtime = GetRunTime(&tbuf);
	query_log[1].runCount++;
	query_log[1].runTime += runtime;
...
*/
/* goda [ */
	tempnext_o_id = d_next_o_id + 1;
	sprintf(tempquery,"UPDATE district SET d_next_o_id = %d WHERE d_id = %d AND d_w_id = %d",tempnext_o_id,d_id,w_id);

	GetStartTime(&tbuf);
	if(mysql_query(ctx[t_num],tempquery)) goto sqlerr;
	runtime = GetRunTime(&tbuf);
	query_log[2].runCount++;
	query_log[2].runTime += runtime;
/* ] goda */
...
#endif
/* goda [ */
	sprintf(tempquery,"INSERT INTO orders (o_id, o_d_id, o_w_id, o_c_id, o_entry_d, o_ol_cnt, o_all_local) VALUES(%d, %d, %d, %d, '%s', %d, %d)",o_id,d_id,w_id,c_id,datetime,o_ol_cnt,o_all_local);

	GetStartTime(&tbuf);
	if(mysql_query(ctx[t_num],tempquery)) goto sqlerr;
	runtime = GetRunTime(&tbuf);
	query_log[3].runCount++;
	query_log[3].runTime += runtime;
/* ] goda */
...
/* goda [ */
	sprintf(tempquery,"INSERT INTO new_orders (no_o_id, no_d_id, no_w_id) VALUES (%d,%d,%d)",o_id,d_id,w_id);

	GetStartTime(&tbuf);
	if(mysql_query(ctx[t_num],tempquery)) goto sqlerr;
	runtime = GetRunTime(&tbuf);
	query_log[4].runCount++;
	query_log[4].runTime += runtime;
...
#endif
/* goda [ */
		sprintf(tempquery,"SELECT i_price, i_name, i_data FROM item WHERE i_id = %d",ol_i_id);
		GetStartTime(&tbuf);
		if(mysql_query(ctx[t_num],tempquery)) goto sqlerr;
		runtime = GetRunTime(&tbuf);
		query_log[5].runCount++;
		query_log[5].runTime += runtime;
...
#endif
/* goda [ */
		sprintf(tempquery,"SELECT s_quantity, s_data, s_dist_01, s_dist_02, s_dist_03, s_dist_04, s_dist_05, s_dist_06, s_dist_07, s_dist_08, s_dist_09, s_dist_10 FROM stock WHERE s_i_id = %d AND s_w_id = %d",ol_i_id,ol_supply_w_id);
//printf("\nno6 query:%s\n",tempquery);

		GetStartTime(&tbuf);
		if(mysql_query(ctx[t_num],tempquery)) goto sqlerr;
		runtime = GetRunTime(&tbuf);
		query_log[6].runCount++;
		query_log[6].runTime += runtime;
...
#endif
/* goda [ */
		sprintf(tempquery,"UPDATE stock SET s_quantity = %d WHERE s_i_id = %d AND s_w_id = %d",s_quantity,ol_i_id,ol_supply_w_id);

		GetStartTime(&tbuf);
		if(mysql_query(ctx[t_num],tempquery)) goto sqlerr;
		runtime = GetRunTime(&tbuf);
		query_log[7].runCount++;
		query_log[7].runTime += runtime;
/* ] goda */

		ol_amount = (int)(ol_quantity * i_price * (1 + (w_tax / 100) + (d_tax/100)) * (1 - (c_discount/100)));
...
*/
/* goda [ */
		sprintf(tempquery,"INSERT INTO order_line (ol_o_id, ol_d_id, ol_w_id, ol_number, ol_i_id, ol_supply_w_id, ol_quantity, ol_amount, ol_dist_info) VALUES (%d,%d,%d,%d,%d,%d,%d,%d,'%s')",o_id,d_id,w_id,ol_number,ol_i_id,ol_supply_w_id,ol_quantity,ol_amount,ol_dist_info);

		GetStartTime(&tbuf);
		if(mysql_query(ctx[t_num],tempquery)) goto sqlerr;
		runtime = GetRunTime(&tbuf);
		query_log[8].runCount++;
		query_log[8].runTime += runtime;
/* ] goda */
