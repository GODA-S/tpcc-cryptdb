...
struct runtime_log query_log[35];
struct runtime_log query_sum[8];
...
*/
  ...
  /* (goda) set up each query_log */
  for ( i=0;i<35;i++ ){
	query_log[i].queryNo = i;
	query_log[i].runCount = 0;
	query_log[i].runTime = 0;
  }
  /* (goda) set query type */
	sprintf(query_log[0].queryType,"SELECT_JOIN");
	sprintf(query_log[1].queryType,"SELECT");
	sprintf(query_log[2].queryType,"UPDATE_INC");
	sprintf(query_log[3].queryType,"INSERT");
	sprintf(query_log[4].queryType,"INSERT");
	sprintf(query_log[5].queryType,"SELECT");
	sprintf(query_log[6].queryType,"SELECT");
	sprintf(query_log[7].queryType,"UPDATE_SET");
	sprintf(query_log[8].queryType,"INSERT");
	sprintf(query_log[9].queryType,"UPDATE_INC");
	sprintf(query_log[10].queryType,"SELECT");
	sprintf(query_log[11].queryType,"UPDATE_INC");
	sprintf(query_log[12].queryType,"SELECT");
	sprintf(query_log[13].queryType,"SELECT");
	sprintf(query_log[14].queryType,"SELECT");
	sprintf(query_log[15].queryType,"SELECT");
	sprintf(query_log[16].queryType,"SELECT");
	sprintf(query_log[17].queryType,"UPDATE_SET");
	sprintf(query_log[18].queryType,"UPDATE_SET");
	sprintf(query_log[19].queryType,"INSERT");
	sprintf(query_log[20].queryType,"SELECT");
	sprintf(query_log[21].queryType,"SELECT");
	sprintf(query_log[22].queryType,"SELECT");
	sprintf(query_log[23].queryType,"SELECT");
	sprintf(query_log[24].queryType,"SELECT");
	sprintf(query_log[25].queryType,"SELECT");
	sprintf(query_log[26].queryType,"DELETE");
	sprintf(query_log[27].queryType,"SELECT");
	sprintf(query_log[28].queryType,"UPDATE_SET");
	sprintf(query_log[29].queryType,"UPDATE_SET");
	sprintf(query_log[30].queryType,"SELECT_SUM");
	sprintf(query_log[31].queryType,"UPDATE_INC");
	sprintf(query_log[32].queryType,"SELECT");
	sprintf(query_log[33].queryType,"SELECT_RANGE");
	sprintf(query_log[34].queryType,"SELECT_RANGE");

  /* (goda) set up query_sum */
  for(i=0;i<8;i++){
    query_sum[i].queryNo = -1;
    query_sum[i].runCount = 0;
    query_sum[i].runTime = 0;
  }
  sprintf(query_sum[0].queryType,"SELECT");
  sprintf(query_sum[1].queryType,"SELECT_JOIN");
  sprintf(query_sum[2].queryType,"SELECT_RANGE");
  sprintf(query_sum[3].queryType,"SELECT_SUM");
  sprintf(query_sum[4].queryType,"INSERT");
  sprintf(query_sum[5].queryType,"DELETE");
  sprintf(query_sum[6].queryType,"UPDATE_SET");
  sprintf(query_sum[7].queryType,"UPDATE_INC");
...
// goda test  for( t_num=0; t_num < 1; t_num++ ){
  ...
  printf("---------------------------------\n");
  printf("query detail\n");
  printf("No | Count | Time(ms) | time/query(ms)\n");
  for(i=0;i<35;i++){
    printf("%3d : %5d : %11.6f : %f\n",query_log[i].queryNo,query_log[i].runCount,query_log[i].runTime,query_log[i].runTime / query_log[i].runCount);
  }

/* sumarize query_log -> query_sum
  0:SELECT
  1:SELECT_JOIN
  2:SELECT_RANGE
  3:SELECT_SUM
  ...
  for(i=0;i<35;i++){
	for(k=0;k<8;k++){
		if(strcmp(query_log[i].queryType,query_sum[k].queryType) == 0){
			query_sum[k].runCount += query_log[i].runCount;
			query_sum[k].runTime += query_log[i].runTime;
			break;
		}
		if(k==7){
			printf("UNKNOWN_QUERY_TYPE!!\n");
			printf("%3d : %7d : %15.6f : %f\n",query_log[i].queryNo,query_log[i].runCount,query_log[i].runTime,query_log[i].runTime / query_log[i].runCount);
		}
	}
  }

  printf("\n\n-----QUERY TYPE SUMMARY -----\n");
  printf("  queryType | runCount | runTime | runTime per query(ms)\n");
  for(i=0;i<8;i++){
    printf("%12s : %5d : %11.6f : %f\n",query_sum[i].queryType,query_sum[i].runCount,query_sum[i].runTime,query_sum[i].runTime / query_sum[i].runCount);
  }
